                 ...
                 r,r_star,uj1,u0_mean_prior,u1_mean_prior,Sigma0,Sigma1, n_iter,k,burnin) 
{
  ...
  for (i in 1:n_iter) {
    ...
    rho = rgamma(p,shape = rho_rate_post,rate = rho_rate_post)
    RHO[,i] = rho
    ...
    for(j in 1:p) {
      ...
      if(lengh_k>0) {
        ...
        Xi0j_tilde = diag(xi0j_tilde, lengh_k , lengh_k )
        O0j_tilde = kappa0j_tilde/xi0j_tilde+u00[h_j]
        V0j_tilde = crossprod(U0j_tilde,Xi0j_tilde)%*%U0j_tilde +Sigma0_tilde_inv[[h_j]] 
        V0j_tilde_inv=solve(V0j_tilde)
        u0[[h_j]][j_j,-1] = mvrnorm(1,V0j_tilde_inv%*%(crossprod(U0j_tilde,Xi0j_tilde)%*%O0j_tilde+Sigma0_tilde_inv[[h_j]]%*%(u0_mean_prior_1[[h_j]]+Sigma0_part1[[h_j]]%*%(u0_star[[h_j]][j_j,-1]-u0_mean_prior_2[[h_j]]) ) ),V0j_tilde_inv)
      } else {
    ...
    for(j in 1:p) {
      ...
      if(lengh_k>0) {
        ...
        Xi0j_star = diag(xi0j_star, lengh_k , lengh_k )
        O0j_star = kappa0j_star/xi0j_star+u00_star[h_j]
        V0j_star = crossprod(U0j_star,Xi0j_star)%*%U0j_star +Sigma0_star_inv[[h_j]] 
        V0j_star_inv=solve(V0j_star)
        u0_star[[h_j]][j_j,-1] = mvrnorm(1,V0j_star_inv%*%(crossprod(U0j_star,Xi0j_star)%*%O0j_star+Sigma0_star_inv[[h_j]]%*%(u0_mean_prior_2[[h_j]]+Sigma0_part2[[h_j]]%*%(u0[[h_j]][j_j,-1]-u0_mean_prior_1[[h_j]]) ) ),V0j_star_inv)
      } else {
    ...
    for(j in 1:p) {
      ...
      if(lengh_k>0) {
        ...
        Xi1j_tilde = diag(xi1j_tilde,lengh_k,lengh_k)  
        O1j_tilde = kappa1j_tilde/xi1j_tilde+u10[h_j]
        V1j_tilde = crossprod(U1j_tilde,Xi1j_tilde)%*%U1j_tilde +Sigma1_tilde_inv[[h_j]] 
        V1j_tilde_inv=solve(V1j_tilde)
        u1[[h_j]][j_j,-1] = mvrnorm(1,V1j_tilde_inv%*%(crossprod(U1j_tilde,Xi1j_tilde)%*%O1j_tilde+Sigma1_tilde_inv[[h_j]]%*%(u1_mean_prior_1[[h_j]]+Sigma1_part1[[h_j]]%*%(u1_star[[h_j]][j_j,-1]-u1_mean_prior_2[[h_j]]) ) ),V1j_tilde_inv)
      } else {
    ...
    for(j in 1:p) {
      ...
      if(lengh_k>0) {
        ...
        Xi1j_star = diag(xi1j_star, lengh_k , lengh_k )
        O1j_star = kappa1j_star/xi1j_star+u10_star[h_j]
        V1j_star = crossprod(U1j_star,Xi1j_star)%*%U1j_star +Sigma1_star_inv[[h_j]] 
        V1j_star_inv=solve(V1j_star)
        u1_star[[h_j]][j_j,-1] = mvrnorm(1,V1j_star_inv%*%(crossprod(U1j_star,Xi1j_star)%*%O1j_star+Sigma1_star_inv[[h_j]]%*%(u1_mean_prior_2[[h_j]]+Sigma1_part2[[h_j]]%*%(u1[[h_j]][j_j,-1]-u1_mean_prior_1[[h_j]]) ) ),V1j_star_inv)
      } else {
    ...
    if(i>burnin) {
      ...
      psi_i = w%*%z+m
      loglikelihood_x = c(loglikelihood_x,n*sum(log(rho))/2-sum((X-psi_i)^2*rho)/2-n*p*log(2*pi)/2) #-n*p*log(2*pi)/2
      psi = psi + psi_i 
    }
